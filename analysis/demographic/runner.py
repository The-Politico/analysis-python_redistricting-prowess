...
from analysis.demographic.data_prep import prepare_tract_data
from analysis.demographic.tract_coverage import process_tract_coverage
...
def run_prep(state_raw, district_type, plan_only=False):
    start = time.time()
    prepare_tract_data(state_raw, district_type, plan_only=plan_only)
    end = time.time()

    return end - start, []


def run_geosplit(state_raw, district_type, plan_only=False):
    start = time.time()
    sts = process_tract_coverage(state_raw, district_type, plan_only=plan_only)
    end = time.time()
...
def run_join(state_abbr, cursor, plan_only=False):
    ...
    racial_breakouts_join_script = load_sql_file(
        "queries/v1/102a__with-racial-breakouts.sql"
    )
    education_levels_join_script = load_sql_file(
        "queries/v1/102b__with-education-levels.sql"
    )
    if not plan_only:
        cursor.execute(racial_breakouts_join_script)
        cursor.execute(education_levels_join_script)

    all_queries.append(racial_breakouts_join_script)
    all_queries.append(education_levels_join_script)
...
def run_calculation(state_abbr, cursor, plan_only=False):
    ...
    gather_script = load_sql_file("queries/v1/103a__majorities-only.sql")
    if not plan_only:
        cursor.execute(gather_script)
    all_queries.append(gather_script)

    cull_script = load_sql_file("queries/v1/103b__significant-minorities.sql")
    if not plan_only:
        cursor.execute(cull_script)
    all_queries.append(cull_script)

    group_script = load_sql_file("queries/v1/103c__grouped-minorities.sql")
    if not plan_only:
        cursor.execute(group_script)
    all_queries.append(group_script)

    gwt_script = load_sql_file("queries/v1/103d__grouped_with_threshold.sql")
    if not plan_only:
        cursor.execute(gwt_script)
    all_queries.append(gwt_script)

    thld1_script = load_sql_file("queries/v1/103e__main-count-90-percent.sql")
    if not plan_only:
        cursor.execute(thld1_script)
    all_queries.append(thld1_script)

    thld2_script = load_sql_file("queries/v1/103f__main-count-95-percent.sql")
    if not plan_only:
        cursor.execute(thld2_script)
    all_queries.append(thld2_script)

    thld3_script = load_sql_file("queries/v1/103g__main-count-97-percent.sql")
    if not plan_only:
...
def run_assembly(state_abbr, cursor, plan_only=False):
    ...
    lcw_script = load_sql_file("queries/v1/104a__larger-coverage-wins.sql")
    if not plan_only:
        cursor.execute(lcw_script)
    all_queries.append(lcw_script)

    thld1_script = load_sql_file("queries/v1/104b__threshold-90-percent.sql")
    if not plan_only:
        cursor.execute(thld1_script)
    all_queries.append(thld1_script)

    thld2_script = load_sql_file("queries/v1/104c__threshold-95-percent.sql")
    if not plan_only:
        cursor.execute(thld2_script)
    all_queries.append(thld2_script)

    thld3_script = load_sql_file("queries/v1/104d__threshold-97-percent.sql")
    if not plan_only:
        cursor.execute(thld3_script)
    all_queries.append(thld3_script)

    pptn1_script = load_sql_file(
        "queries/v1/104e__proportional-90-percent.sql"
    )
    if not plan_only:
    ...
    pptn2_script = load_sql_file(
        "queries/v1/104f__proportional-95-percent.sql"
    )
    if not plan_only:
    ...
    pptn3_script = load_sql_file(
        "queries/v1/104g__proportional-97-percent.sql"
    )
    if not plan_only:
...
def run_formatting(state_abbr, cursor, plan_only=False):
    ...
    lcw_script = load_sql_file("queries/v1/105a__larger-coverage-wins0.sql")
    if not plan_only:
        cursor.execute(lcw_script)
    all_queries.append(lcw_script)

    thld1_script = load_sql_file("queries/v1/105b__threshold-90-percent.sql")
    if not plan_only:
        cursor.execute(thld1_script)
    all_queries.append(thld1_script)

    thld2_script = load_sql_file("queries/v1/105c__threshold-95-percent.sql")
    if not plan_only:
        cursor.execute(thld2_script)
    all_queries.append(thld2_script)

    thld3_script = load_sql_file("queries/v1/105d__threshold-97-percent.sql")
    if not plan_only:
        cursor.execute(thld3_script)
    all_queries.append(thld3_script)

    pptn1_script = load_sql_file(
        "queries/v1/105e__proportional-90-percent.sql"
    )
    if not plan_only:
        cursor.execute(pptn1_script)
    all_queries.append(pptn1_script)
    ...
    pptn2_script = load_sql_file(
        "queries/v1/105f__proportional-95-percent.sql"
    )
    if not plan_only:
        cursor.execute(pptn2_script)
    all_queries.append(pptn2_script)
    ...
    pptn3_script = load_sql_file(
        "queries/v1/105g__proportional-97-percent.sql"
    )
    if not plan_only:
        cursor.execute(pptn3_script)
    all_queries.append(pptn3_script)
...
def run_comparison(state_abbr, cursor, district_type="old", plan_only=False):
    ...
    comparison_script = load_sql_file("queries/v1/106__compared-margins.sql")
...
def run_full_demographic_analysis(
    ...
    state_obj, db, cursor = load_state_with_db(
